...
import org.wso2.carbon.apimgt.gateway.mediators.RequestChangeMediator;
...
public class AIAPIMediator extends AbstractMediator implements ManagedLifecycle {
    ...
    private void processInboundRequest(MessageContext messageContext,
        ...
        // Check for RequestChangeMediator configuration and execute if present
        String requestChangeConfigs = (String) messageContext.getProperty("requestChangeConfigs");
        if (requestChangeConfigs != null && !requestChangeConfigs.trim().isEmpty()) {
            try {
                RequestChangeMediator requestChangeMediator = new RequestChangeMediator();
                requestChangeMediator.setRequestChangeConfigs(requestChangeConfigs);
                if (!requestChangeMediator.mediate(messageContext)) {
                    log.warn("RequestChangeMediator returned false, continuing with normal flow");
                } else {
                    // RequestChangeMediator successfully processed the request
                    // Continue with AIAPIMediator but with modified values
                    if (log.isDebugEnabled()) {
                        log.debug("RequestChangeMediator processed successfully, continuing with modified values");
                    }
                }
            } catch (Exception e) {
                log.error("Error executing RequestChangeMediator", e);
            }
        }
        ...
        // Don't override endpoint if RequestChangeMediator already set it
        if (requestChangeConfigs == null || requestChangeConfigs.trim().isEmpty()) {
            messageContext.setProperty(APIConstants.AIAPIConstants.TARGET_ENDPOINT,
                    APIConstants.AIAPIConstants.DEFAULT_ENDPOINT);
        } else {
            // RequestChangeMediator set a custom endpoint, keep using it
            if (log.isDebugEnabled()) {
                log.debug("Using RequestChangeMediator endpoint: " + messageContext.getProperty(APIConstants.AIAPIConstants.TARGET_ENDPOINT));
            }
        }
    ...
    private void processOutboundResponse(MessageContext messageContext,
        ...
        // Check if RequestChangeMediator was used
        String requestChangeConfigs = (String) messageContext.getProperty("requestChangeConfigs");
        boolean isRequestChanged = requestChangeConfigs != null && !requestChangeConfigs.trim().isEmpty();
        
        // If RequestChangeMediator was used, continue normal processing with modified values
        if (isRequestChanged) {
            if (log.isDebugEnabled()) {
                log.debug("RequestChangeMediator was used, continuing with modified endpoint values");
            }
        }
        ...
        if (handleSuccessfulResponse(messageContext, statusCode, providerConfigs, roundRobinConfigs, llmRouteConfigs, failoverConfigs, isRequestChanged)) {
            return;
        }
        ...
        // Handle RequestChangeMediator - just exit normally
        if (isRequestChanged) {
            if (log.isDebugEnabled()) {
                log.debug("RequestChangeMediator processing completed, exiting");
            }
            messageContext.setProperty(APIConstants.AIAPIConstants.TARGET_ENDPOINT,
                    APIConstants.AIAPIConstants.EXIT_ENDPOINT);
            return;
    ...
    private boolean handleSuccessfulResponse(MessageContext messageContext, int statusCode,
        ...
        //Request is still in progress, so no response yet.                                            
        if (statusCode <= 0) {
            return false;
        }                                        
        ...
        // Handle RequestChangeMediator response  
        if (isRequestChanged) {
            if (log.isDebugEnabled()) {
                log.debug("RequestChangeMediator was used, handling response normally");
            }
            messageContext.setProperty(APIConstants.AIAPIConstants.TARGET_ENDPOINT,
                    APIConstants.AIAPIConstants.EXIT_ENDPOINT);
            return true;
